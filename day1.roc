...
main =
    ...
    (leftList, rightList) = List.walk listOfBoth ([], []) (\(leftSoFar, rightSoFar), next -> 
    ...
    distances = List.map2 sortedLeft sortedRight (\left,right -> 
        Num.abs (left - right)
    )
    ...
    similarityScores = List.map leftList (\nextLeft -> 
...
addToCountsDict = \dictSoFar, key -> 
    Dict.update dictSoFar key (\maybeVal -> 
        when maybeVal is 
            Err Missing -> Ok 1 
            Ok count -> Ok (count + 1)
    )