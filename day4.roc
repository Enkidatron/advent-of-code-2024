...
part1 = \text -> 
    ...
    width = List.map matrix List.len 
        |> List.max 
        |> Result.withDefault 0 
    List.joinMap (List.range {start: At 0, end: Before height}) (\h -> 
            List.map (List.range { start: At 0, end: Before width})
...
findCornersFromHere = \matrix, h, w -> 
    ...
    , [(MinusOne, PlusOne), (Keep, Keep), (PlusOne, MinusOne)] ]
        |> List.keepOks (\offsets ->
            List.mapTry offsets (\(hOffset, wOffset) -> 
